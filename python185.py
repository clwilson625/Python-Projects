...
with conn:
      ...
      #executes a SELECT statement to grab all the info from the table
      cur.execute("SELECT * FROM tbl_name")

      #grabs the info that the SELECT statement saves to the cursor, and
      #saves it to a variable named "results"

      results = cur.fetchall()
      #prints the results, which is all the info in the table
      print(results)
conn.close()
